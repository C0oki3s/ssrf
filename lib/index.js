...
async function resolveAll (hostname) {
  ...
  // Fallback: dns.lookup (gives system resolver results)
  if (!results.length) {
    ...
    if (Array.isArray(addresses)) {
      for (const addr of addresses) {
        if (addr && addr.address) results.push(addr.address)
      }
    }
  }
  ...
  if (host.includes('.')) {
    ...
    if (parts.every(p => p.length > 0)) {
      ...
      if (nums.every(x => x !== null && x >= 0)) {
        let dword = null
        if (nums.length === 4) {
          if (nums.every(x => x <= 255)) dword = ((nums[0] << 24) | (nums[1] << 16) | (nums[2] << 8) | nums[3]) >>> 0
        } else if (nums.length === 3) {
          if (nums[0] <= 255 && nums[1] <= 255 && nums[2] <= 0xFFFF) dword = ((nums[0] << 24) | (nums[1] << 16) | nums[2]) >>> 0
